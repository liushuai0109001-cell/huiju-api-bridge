...
2. 语言、图片、视频 Base URL 已固定为 `https://huiju.v888.art`，图床固定为 `https://huiju.v888.art/upload`；用户只需填写 API Key，然后点击“获取模型”并从下拉框选择目标模型。
3. 点击“保存配置”。服务会自动重启并应用新配置。
4. 图片比例、视频比例和视频时长都使用界面选项配置，不需要填写 JSON。
   视频时长始终以外接软件中的 15 秒或 30 秒选项为准，并覆盖洛水请求值。
5. 点击“启动洛水”，软件会携带外接代理环境变量启动洛水。

程序打开后自动监听 `127.0.0.1:5400` 和 `127.0.0.1:8000`。关闭窗口会停止两个监听服务。

模型和图床 URL 在界面中只读，配置文件中的其他 URL 也会在读取和保存时自动恢复为固定地址。
...
客户端会并发探测 `download_urls` 中的多个镜像，优先下载最先响应的地址；GitHub 慢或不可达时会自动尝试 CNB。下载完成后自动校验 SHA256，启动独立更新进程，等待主程序退出后自动替换并重启客户端。`config.json`、`license.dat` 和日志会保留。发行包不包含授权私钥、客户授权文件或 API Key。
